...
#import "ChannelFilter/ChannelFilterSettingsController.h"
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return 1; // section 2: ChannelFilter
}
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            cell.textLabel.text = @"チャンネルフィルタ（常時ON）";
            cell.detailTextLabel.text = @"ホワイトリストの同期状態を確認する";
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        if (indexPath.row == 0) {
            ChannelFilterSettingsController *cfVC = [[ChannelFilterSettingsController alloc] initWithStyle:UITableViewStyleInsetGrouped];
            UINavigationController *cfNav = [[UINavigationController alloc] initWithRootViewController:cfVC];
            cfNav.modalPresentationStyle = UIModalPresentationFullScreen;
            [self presentViewController:cfNav animated:YES completion:nil];
        }
    }
}
